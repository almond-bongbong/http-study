...
웹서버는 정적인 컨텐츠(HTML, CSS, JS, 이미지 등)를 제공하며, WAS는 동적인 컨텐츠(애플리케이션 로직)를 처리합니다.
이러한 역할 분담은 다음과 같은 이점을 제공합니다.

- 웹서버와 WAS를 분리함으로써, 부하를 분산할 수 있습니다.
- 웹서버는 방화벽 기능을 수행하여, 악성요청이나 DDoS 공격을 차단할 수 있습니다. 또한, SSL/TLS 암호화와 복호화를 처리하는 부하를 WAS에서 분산할 수 있습니다.
- WAS는 웹서버와 독립적으로 확장할 수 있습니다.
...
GET, POST 방식이 있습니다. [🔗](https://www.encar.com/ms/ms_search.do?method=pw) [🔗](https://codesandbox.io/s/flamboyant-feistel-g2fjfi?file=/src/App.js)
